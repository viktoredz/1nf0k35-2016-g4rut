...
<script type="text/javascript">
	...
	$(function () {	
	   ...
	   var source = {
		...
		filter: function(){
			$("#jqxgridBRetur").jqxGrid('updatebounddata', 'filter');
		},
		sort: function(){
			$("#jqxgridBRetur").jqxGrid('updatebounddata', 'sort');
		},
		root: 'Rows',
        pagesize: 10,
		...
		{		
			...
			columns: [
				{ text: 'Detail', align: 'center', filtertype: 'none', sortable: false, width: '5%', cellsrenderer: function (row) {
				    var dataRecord = $("#jqxgridBRetur").jqxGrid('getrowdata', row)
				    if((dataRecord.id_mst_inv_barang_habispakai!=null)&&(dataRecord.tgl_opname!="<?php echo date('Y-m-d');?>")){
						return "<div style='width:100%;padding-top:2px;text-align:center'><a href='javascript:void(0);'><img border=0 src='<?php echo base_url(); ?>media/images/16_view.gif' onclick='detail(\""+dataRecord.id_inv_inventaris_habispakai_opname+"\",\""+dataRecord.id_mst_inv_barang_habispakai+"\",\""+dataRecord.batch+"\");'></a></div>";
					}else{
	...
	$("#btn-export").click(function(){
		...
		var sortdatafield = $("#jqxgridBRetur").jqxGrid('getsortcolumn');
