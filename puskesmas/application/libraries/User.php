	...
	class User {
		...
		function User()
		{
			...
			if($this->obj->session->userdata('username') != ""){
				...
				//sesuaikan dengan database puskesmas kecamatan
				/*$database_list = $this->obj->config->item('database_list');
				$database_name = substr($this->obj->session->userdata('puskesmas'),0,7);
				if(in_array($database_name, $database_list)){
					$this->obj->load->database($database_name, FALSE, TRUE);
				}*/
		...
		function login($type=""){
			...
			/*$database_list = $this->obj->config->item('database_list');
			$database_name = substr($this->obj->input->post('kode'),0,7);
			if(in_array($database_name, $database_list)){
				$this->obj->load->database($database_name, FALSE, TRUE);
			}*/
