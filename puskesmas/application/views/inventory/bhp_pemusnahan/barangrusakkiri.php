...
<script>
	$(function(){ 
	   var source_rusakkiri = {
		...
		url: "<?php echo site_url('inventory/bhp_pemusnahan/json_rusakkiri/'.$kode_rusak.'/'.$tgl_opname); ?>",
		cache: false,
		updateRow: function (rowID, rowData, commit) {
         },
		filter: function(){
			$("#jqxgrid_barang_rusak_kiri").jqxGrid('updatebounddata', 'filter');
		},
		sort: function(){
			$("#jqxgrid_barang_rusak_kiri").jqxGrid('updatebounddata', 'sort');
		},
		root: 'Rows',
        pagesize: 10,
        beforeprocessing: function(data){		
			if (data != null){
				source_rusakkiri.totalrecords = data[0].TotalRows;					
			}
		}
		};		
		var dataadapter_rusakkiri = new $.jqx.dataAdapter(source_rusakkiri, {
			loadError: function(xhr, status, error){
				alert(error);
			}
		});
     
		$("#jqxgrid_barang_rusak_kiri").jqxGrid(
		{	
			width: '100%',
			selectionmode: 'singlerow',
			source: dataadapter_rusakkiri, theme: theme,columnsresize: true,showtoolbar: false, pagesizeoptions: ['10', '25', '50', '100'],
		...
		$('#clearfilteringbutton').click(function () {
			$("#jqxgrid_barang_rusak_kiri").jqxGrid('clearfilters');
		});
	...
	function close_popup_master(){
		$("#popup_barang_rusak_kiri").jqxWindow('close');
	...
	function del_barang(id_barang,kode_batch){
		...
		if(confirms == true){
			$.post("<?php echo base_url().'inventory/bhp_pemusnahan/dodelpermohonan/'.$kode_rusak.'/'; ?>" + id_barang+'/'+kode_batch,  function(){
	...
	function pilih_opname_master(jenis){
		...
		$("#popup_barang_rusak_kiri #popup_content_rusak_kiri").html("<div style='text-align:center'><br><br><br><br><img src='<?php echo base_url();?>media/images/indicator.gif' alt='loading content.. '><br>loading</div>");
		$.get("<?php echo base_url().'inventory/bhp_pemusnahan/add_barang_opnamemaster/'.$kode_rusak.'/'?>"+idjenis, function(data) {
			$("#popup_content_rusak_kiri").html(data);
		});
		$("#popup_barang_rusak_kiri").jqxWindow({
			theme: theme, resizable: false,
			width: 600,
			height: 400,
			isModal: true, autoOpen: false, modalOpacity: 0.2
		});
		$("#popup_barang_rusak_kiri").jqxWindow('open');
	}
</script>

<div id="popup_barang_rusak_kiri" style="display:none">
	<div id="popup_title_rusak_kiri">Data Opname Barang Master</div>
	<div id="popup_content_rusak_kiri">&nbsp;</div>
</div>
...
<div>
	...
	<div class="box-body">
		<div style="width:100%;">
	        <div id="jqxgrid_barang_rusak_kiri"></div>
		</div>
	</div>
</div>