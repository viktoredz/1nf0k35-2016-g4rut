...
class Penyusutan extends CI_Controller {
	...
	function api_data_sts_general(){
		...
		if(!empty($this->session->userdata('puskesmas')) and  $this->session->userdata('puskesmas') != '0'){
			$data['ambildata'] = $this->penyusutan_model->get_data_keu_sts_general($this->session->userdata('puskesmas'));
			foreach($data['ambildata'] as $d){
				$txt = $this->convert_tgl($d["tgl"])." \t ".$d["nomor"]." \t ".$d["total"]." \t ".$d["status"]." \t <a href=\"".base_url()."keuangan/penyusutan/detail/".$d['tgl']."\"><img border=0 src='".base_url()."media/images/16_edit.gif'></a>  \t".($d['status'] == "buka" ? "  <a onclick=\"doDeleteSts('".$d['tgl']."')\" href=\"#\" ><img border=0 src='".base_url()."media/images/16_del.gif'></a>" : "<img border=0 src='".base_url()."media/images/16_lock.gif'>").  "\n ";				
				echo $txt;
			}
		}		
	}
	...
	function api_data_sts_detail($id){
		...
		if(!empty($this->session->userdata('puskesmas')) and  $this->session->userdata('puskesmas') != '0'){
			$data['ambildata'] = $this->penyusutan_model->get_data_puskesmas_isi_sts($id);
	...
	function sts_tgl(){
		if(!$this->penyusutan_model->cek_sts_tgl()){
	...
	function sts_nomor(){
		if(!$this->penyusutan_model->cek_sts_nomor()){
	...
	function detail($id){
		...
		$data['data_sts'] = $this->penyusutan_model->get_data_sts($id, $this->session->userdata('puskesmas'));
		$data['data_sts_total'] = $this->penyusutan_model->get_data_sts_total($id, $this->session->userdata('puskesmas'));
		$data['nomor'] = $this->generate_nomor(date("Y-m-d H:i:s"));		
		$data['id'] = $id;
		$data['content'] = $this->parser->parse("keuangan/penyusutan/detail_sts",$data,true);		
	...
	function reopen(){
		$this->authentication->verify('keuangan','edit');
		$this->penyusutan_model->reopen();
		echo "buka lagi";
	}
	
	function update_volume(){		
		$this->authentication->verify('keuangan','edit');		
		echo $this->penyusutan_model->update_volume();
	}
	
	function tutup_sts(){		
		$this->authentication->verify('keuangan','edit');		
		$this->penyusutan_model->tutup_sts();
		$this->penyusutan_model->rekap_sts_rekening();
		#redirect(base_url().'keuangan/penyusutan/general', 'refresh');
	}
	
	function update_ttd(){		
		$this->authentication->verify('keuangan','edit');		
		if(empty($this->input->post('delete'))){
			$this->penyusutan_model->update_ttd();
		}
		...
		if($this->form_validation->run()== FALSE){
			...
			redirect(base_url().'keuangan/penyusutan/detail/'.$this->input->post('id_sts'));
		}else{
	...
	function detail_sts_export(){
		...
		$activity = $this->penyusutan_model->get_data_for_export('keu_sts', array('keu_sts.id_sts'=>$id))->result();
		...
		$data_puskesmas						= $this->penyusutan_model->get_data_row($id);
		$nama_puskesmas						= $this->penyusutan_model->get_data_nama($data_puskesmas['code_cl_phc']);
	...
	function koderekening(){
		...
		$data['content'] = $this->parser->parse("keuangan/penyusutan/kode_rekening",$data,true);						
	...
	function kode_rekening_json(){
		...
		$rows_all = $this->penyusutan_model->get_data_kode_rekening_all();
		...
		$rows = $this->penyusutan_model->get_data_kode_rekening_all($this->input->post('recordstartindex'), $this->input->post('pagesize'));
		$data = array();
		foreach($rows as $r) {
			$data[] = array(
				'code'			=> $r->code,
